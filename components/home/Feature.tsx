  ...
  return (
    <section className="bg-white">
      <div
        ...
        "
      >
        {/* Left Column (Mobile second) — Image */}
        <div className="relative flex flex-col justify-center items-center min-h-[300px] sm:min-h-[400px] lg:min-h-[600px] p-4">
          <div className="absolute w-[72%] h-[72%] bg-[#D9F2E6] rounded-full blur-3xl opacity-70 -z-10" />

          <div className="relative w-full max-w-[570px] aspect-[4/3] rounded-xl overflow-hidden shadow-2xl">
            <Image
              key={activeFeature.id}
              src={activeFeature.imageSrc || "/placeholder.svg"}
              alt={activeFeature.title}
              fill
              sizes="(max-width: 1024px) 80vw, 25.6vw"
        ...
        {/* Right Column (Mobile first) — Details */}
        <div className="flex flex-col gap-8">
          <div className="space-y-4">
            <h2 className="text-2xl sm:text-3xl lg:text-4xl font-normal leading-snug">
              ...
              You <br className="hidden sm:block" /> Handle{" "}
          ...
          {/* Feature Boxes */}
          <div className="flex flex-wrap sm:flex-nowrap items-center gap-4 sm:gap-8 mt-5">
            {features.map((feature) => (
              <button
                key={feature.id}
                onClick={() => setActiveFeature(feature)}
                className={cn(
                  "flex flex-col h-[160px] w-full sm:w-[170px] items-start p-6 rounded-xl border-2 transition-all duration-300",
                  activeFeature.id === feature.id
